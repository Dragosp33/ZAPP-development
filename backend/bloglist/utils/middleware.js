...
const requireAuth = async (req, res, next) => {
  ...
  // changes from
  if (token) {
    const accessTokenFound = await Tokens.findOne({
      access_token: token,
    })
    console.log('!!!   FOUND ACCESS TOKEN  !!!', accessTokenFound)
    if (!accessTokenFound) {
      res.clearCookie('access_token')
      res.clearCookie('refresh_token')
      // res.end()
      return res.status(401).json({ error: 'Access token not found in db' })
    }
    if (accessTokenFound && accessTokenFound.valid === false) {
      res.clearCookie('access_token')
      res.clearCookie('refresh_token')
      // res.end()
      /* res.cookie('access_token', '', { expires: new Date(0) })
      res.cookie('refresh_token', '', { expires: new Date(0) })
      console.log('accessTokenFound?', accessTokenFound) */
      invalidateTokens(accessTokenFound)
      return res
        .status(401)
        .json({ error: 'Unauthorized, access token invalid' })
    }
  }
  ...
  if (!refresh) {
    return res.status(401).json({ error: 'Unauthorized, no Refresh' })
  }
  if (!token) {
    console.log('no access token here, looking for a refresh')
    const tokens = await checkToken(refresh)
    console.log('tokens returned: ', tokens)
    if (!tokens) {
      res.cookie('access_token', '', { expires: new Date(0) })
      res.cookie('refresh_token', '', { expires: new Date(0) })
      return res.status(401).json({ error: 'Unauthorized, tokens reused' })
    }
    token = tokens.access
...
const checkToken = async (refreshToken) => {
  ...
  const foundUser = await User.findOne({ _id: userForToken.id })
  if (foundUser) {
    console.log('User found, verify valid', foundUser)
    if (foundUser.verified === false) {
      console.log('user not verified, return null. ')
      return null
    }
  }
